<?php
    ...
    include ('../config/variables.php');
        ...
        $sqlGetProduct = "SELECT $tProduct.id, $tProduct.nombre FROM $tProduct INNER JOIN $tStock ON $tProduct.id = $tStock.producto_id AND $tStock.tienda_id='$store' WHERE ($tProduct.nombre LIKE '%{$query}%' OR $tProduct.codigo_barras LIKE '%{$query}%' ) LIMIT 8 ";
